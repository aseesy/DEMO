...
function createUser(username, email) {
  return db.query('INSERT INTO users (username, email) VALUES ($1, $2)', [username, email]);
}
```
...
async function getUserContext(username) {
  ...
  const result = await db.query('SELECT * FROM user_context WHERE user_id = $1', [
...
async function getUserContext(usernameInput) {
  ...
  const result = await db.query('SELECT * FROM user_context WHERE user_id = $1', [username.value]);
  return result.rows[0];
}
```
...
async function saveMessage(message) {
  ...
  await db.query('INSERT INTO messages (id, room_id, username, text) VALUES ($1, $2, $3, $4)', [
    id,
    roomId,
    username,
    message.text,
...
function canUserAccessRoom(userId, roomId) {
  ...
  return db.query('SELECT * FROM room_members WHERE user_id = $1 AND room_id = $2', [
    userId,
    roomId,
  ]);
}
