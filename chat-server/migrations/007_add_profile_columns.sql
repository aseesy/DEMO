...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'additional_context') THEN
    ALTER TABLE users ADD COLUMN additional_context TEXT;
    RAISE NOTICE 'Added additional_context column to users table';
  END IF;

  -- Add profile_picture column (stores base64 or URL)
