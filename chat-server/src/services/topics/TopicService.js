...
class TopicService {
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        logger.debug('Log message', {
          value: `[TopicService] Regenerated summary for ${topicId}`,
        });
      } catch (error) {
        logger.error('Log message', {
          arg0: `[TopicService] Failed to regenerate ${topicId}:`,
          error: error,
        });
      } finally {
  ...
  async reportInaccurate(topicId, reportedBy, reason) {
    ...
    logger.debug('Log message', {
      value: `[TopicService] Summary reported: ${topicId} by ${reportedBy}: ${reason}`,
    });
  ...
  async getMessageContext(messageId, contextSize = 5) {
    ...
    const targetResult = await pool.query('SELECT * FROM messages WHERE id = $1', [messageId]);
