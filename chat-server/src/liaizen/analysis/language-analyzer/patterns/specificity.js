...
function detect(text) {
  ...
  const hasVagueReference = VAGUE_REFERENCES.some(marker => lowerText.includes(marker));

  // Check for vague complaint patterns
  const hasVagueComplaint = VAGUE_COMPLAINT_PATTERNS.some(pattern => pattern.test(text));

  // Check for vague request patterns
  const hasVagueRequest = VAGUE_REQUEST_PATTERNS.some(pattern => pattern.test(text));

  // Check for specific request patterns
  const hasSpecificRequest = SPECIFIC_REQUEST_PATTERNS.some(pattern => pattern.test(text));

  // Check for specific references
  const hasSpecificReference = SPECIFIC_REFERENCE_PATTERNS.some(pattern => pattern.test(text));

  // Check for concrete items mentioned
  const hasConcreteItem =
    /\b(the|her|his)\s+(inhaler|backpack|homework|lunch|medication|jacket|phone|tablet|charger)\b/i.test(
      text
    );

  // Check for specific time reference
  const hasTimeReference =
    /\b(at|by|before|after|until)\s+\d/i.test(text) || /\b\d{1,2}(:\d{2})?\s*(am|pm)\b/i.test(text);
