...
Respond in JSON:
{
  ...
  "confidence": 0-100
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are an expert in emotional analysis for co-parenting communication. Analyze emotional states, trajectories, and stress patterns. Respond only with valid JSON.',
        },
        {
          role: 'user',
          content: prompt,
    ...
    const maxMomentum = Math.max(
      ...Object.values(state.participants).map(p => p.emotionalMomentum),
      0
    );
...
function calculateEmotionTrend(emotionHistory) {
  ...
  const earlierNegative = emotionHistory
    .slice(-10, -5)
    .filter(e => negativeEmotions.includes(e.emotion)).length;
...
function resetEmotionalState(roomId, username = null) {
  ...
  if (username && state.participants[username]) {
    ...
    state.participants[username].stressLevel = Math.max(
      0,
      state.participants[username].stressLevel - 20
    );
    state.participants[username].emotionalMomentum = Math.max(
      0,
      state.participants[username].emotionalMomentum - 10
    );
  } else {
    // Reset entire room
