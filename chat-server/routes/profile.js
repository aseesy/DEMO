...
router.get('/me', authenticate, async (req, res) => {
    try {
        ...
        const privacyQuery = await db.query(
            'SELECT * FROM user_profile_privacy WHERE user_id = $1',
            [userId]
        );
        
        const privacySettings = privacyQuery.rows.length > 0
            ? privacyQuery.rows[0]
            : null;
...
router.put('/me', authenticate, async (req, res) => {
    try {
        ...
        const backendManagedFields = ['profile_last_updated', 'profile_completion_percentage', 'currentUsername'];
        ...
        const validUpdateKeys = Object.keys(updates).filter(key =>
            allProfileColumns.includes(key) && !backendManagedFields.includes(key)
        );
        ...
        if (validUpdateKeys.length === 0) {
            console.log('DEBUG PUT /api/profile/me - No valid fields! Request keys:', Object.keys(updates));
        ...
        const filledCount = allProfileColumns.filter(col => updatedProfile[col] != null && updatedProfile[col] !== '').length;
 ...
 * Fetches privacy settings for the currently authenticated user.
 */
router.get('/privacy/me', authenticate, async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Internal server error while fetching privacy settings.' });
    }
});
...
router.put('/privacy/me', authenticate, async (req, res) => {
    try {
        ...
        const existingQuery = await db.query(
            'SELECT id FROM user_profile_privacy WHERE user_id = $1',
            [userId]
        );
        ...
        const settingsData = {
            ...
            field_overrides: typeof field_overrides === 'string' ? field_overrides : JSON.stringify(field_overrides || {}),
            updated_at: new Date()
        };
...
router.get('/preview-coparent-view', authenticate, async (req, res) => {
    try {
        ...
        const settings = privacyQuery.rows.length > 0
            ? privacyQuery.rows[0]
            : {
                personal_visibility: 'shared',
                work_visibility: 'private',
        ...
        try {
            const overrides = typeof settings.field_overrides === 'string'
