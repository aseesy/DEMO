...
const GLOBAL_MARKERS = [
  'always', 'never', 'every time', 'constantly', 'forever',
  'completely', 'totally', 'absolutely', 'entirely',
  'all the time', 'without fail', 'no matter what',
  'basically', 'essentially', 'fundamentally'
];

// Specific time/behavior markers
const SPECIFIC_MARKERS = [
  'on tuesday', 'on wednesday', 'on monday', 'on thursday', 'on friday',
  'on saturday', 'on sunday', 'yesterday', 'last week', 'last month',
  'this morning', 'tonight', 'at \\d', 'when you', 'that time',
  'specifically', 'the time when', 'on the \\d'
];
...
function detect(text) {
  ...
  // Find global markers
  const foundGlobalMarkers = GLOBAL_MARKERS.filter(marker =>
  ...
  const hasSpecificBehavior = hasSpecificReference ||
    /\bwhen you (did|said|went|took|picked|dropped|forgot)\b/i.test(text);

  // Detect specific impact
  const hasSpecificImpact = /\b(she|he|they)\s+(missed|was late|couldn't|didn't get|had to)\b/i.test(text) ||
