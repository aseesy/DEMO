...
async function testActivitiesAPI() {
  ...
  try {
    ...
    const childContact = contactsResponse.contacts.find(
      c => c.relationship === 'my child'
    );
    ...
    const getResponse = await httpRequest('GET', `/api/activities/${childContact.id}?username=${TEST_USERNAME}`);
    ...
    const verifyResponse = await httpRequest('GET', `/api/activities/${childContact.id}?username=${TEST_USERNAME}`);
    ...
    const deleteResponse = await httpRequest('DELETE', `/api/activities/${activityId}?username=${TEST_USERNAME}`);

    console.log('✅ Activity deleted:', deleteResponse.message, '\n');

    // Step 7: Verify deletion
    console.log('📋 Step 7: Verifying deletion...');
    const finalResponse = await httpRequest('GET', `/api/activities/${childContact.id}?username=${TEST_USERNAME}`);
