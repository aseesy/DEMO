...
async function testLogin(endpoint, body, expectedStatus, expectedCode, expectedMessage) {
  try {
    ...
    const messageMatch = !expectedMessage || (data.error && data.error.toLowerCase().includes(expectedMessage.toLowerCase()));
...
async function runTests() {
  ...
  if (test1.connectionError) {
    ...
    logTest('Missing email', test1.success, 
      test1.success 
  ...
  console.log('\nTest 2: Missing password');
  const test2 = await testLogin('/api/auth/login', { email: 'test@example.com' }, 400, null, 'required');
  if (test2.connectionError) {
    logTest('Missing password', false, test2.error);
  } else {
    logTest('Missing password', test2.success,
      test2.success 
  ...
  console.log('\nTest 3: Invalid email format');
  const test3 = await testLogin('/api/auth/login', { email: 'notanemail', password: 'test123' }, 400, null, 'valid email');
  if (test3.connectionError) {
    logTest('Invalid email format', false, test3.error);
  } else {
    logTest('Invalid email format', test3.success,
      test3.success 
  ...
  console.log('\nTest 4: Account not found');
  const test4 = await testLogin('/api/auth/login', { email: 'nonexistent@example.com', password: 'anypassword' }, 404, 'ACCOUNT_NOT_FOUND', 'No account found');
  if (test4.connectionError) {
    logTest('Account not found', false, test4.error);
  } else {
    logTest('Account not found', test4.success,
      test4.success 
  ...
  const test5 = await testLogin('/api/auth/login', { email: 'test@example.com', password: 'wrongpassword' }, 401, 'INVALID_PASSWORD', 'Incorrect password');
  if (test5.connectionError) {
    ...
    logTest('Wrong password', test5.success,
      test5.success 
  ...
  const test6 = await testLogin('/api/auth/login', { email: 'oauth@example.com', password: 'anypassword' }, 403, 'OAUTH_ONLY_ACCOUNT', 'Google sign-in');
  if (test6.connectionError) {
    ...
    logTest('OAuth-only account', test6.success,
      test6.success 
