...
function getDisambiguatedDisplay(user, allUsers) {
  const sameName = allUsers.filter(u =>
    u.display_name === user.display_name && u.id !== user.id
  );
...
```json
{
  ...
  "user": { /* partial user data if account was created */ }
}
```
...
*Specification created: 2025-11-25*
