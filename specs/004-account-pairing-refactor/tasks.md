...
- [ ] **tests** directory created for unit tests
...
function AcceptPairingPage() {
  ...
  useEffect(() => {
    ...
    if (code) {
      apiClient
        .post('/api/pairing/validate-code', { code })
...
function PairingStatusWidget() {
  ...
  useEffect(() => {
    // Fetch status
    apiClient.get('/api/pairing/status').then(res => setStatus(res.data));
...
const apiClient = {
  ...
  acceptPairing: pairingCode => axios.post('/api/pairing/accept', { pairingCode }, authHeaders()),

  acceptPairingWithSignup: data => axios.post('/api/pairing/accept-with-signup', data),

  getPairingStatus: () => axios.get('/api/pairing/status', authHeaders()),
...
describe('E2E: Email Invitation - New User', () => {
  test('complete flow from invitation to chat', async () => {
    ...
    const room = await db.query('SELECT * FROM rooms WHERE id = $1', [
      acceptRes.data.pairing.roomId,
    ]);
...
test('code pairing between existing users', async () => {
  ...
  const rooms = await db.query('SELECT * FROM rooms WHERE created_by IN ($1, $2)', [
    userA.id,
    userB.id,
  ]);
...
_Task list for coparentliaizen.com - Better Co-Parenting Through Better Communication_
_Based on Constitution v1.5.0 - Library-First, Test-First, Contract-First_
