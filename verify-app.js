...
async function verifyDashboard() {
  ...
  try {
    ...
    const loginForm = await page.locator('input[type="email"], input[placeholder*="email" i]').first();
    ...
    if (isVisible) {
      ...
      console.log(`  ${highExists ? '❌' : '✅'} High Priority tab: ${highExists ? 'STILL EXISTS (should be removed!)' : 'REMOVED (correct!)'}`);
      ...
      if (openExists) {
        ...
        console.log(`  ${isSelected ? '✅' : '❌'} Open tab is ${isSelected ? 'SELECTED (default)' : 'NOT selected'}`);
      }

      // Count visible tasks
      console.log('🔍 Checking tasks...');
      const tasks = await page.locator('div:has-text("Welcome to LiaiZen"), div:has-text("Complete Your Profile"), div:has-text("Add Your"), div:has-text("Children")').all();
      ...
      console.log(`  ${coparentVisible ? '❌' : '✅'} "Add Your Co-parent" task: ${coparentVisible ? 'VISIBLE (BUG - it\'s completed!)' : 'NOT VISIBLE (correct!)'}`);
      ...
      console.log(`  ${threadsExists ? '✅' : '❌'} Threads section: ${threadsExists ? 'EXISTS' : 'MISSING'}`);
      ...
      if (welcomeExists) {
        ...
        const hasScroll = await modalContent.evaluate(el => el.scrollHeight > el.clientHeight).catch(() => false);
        console.log(`  ${hasScroll ? '✅' : 'ℹ️ '} Modal content: ${hasScroll ? 'SCROLLABLE (has overflow)' : 'fits without scrolling'}`);
      ...
      console.log(`${openExists && completedExists && allExists && !highExists ? '✅' : '⚠️ '} Task filter tabs: ${openExists && completedExists && allExists && !highExists ? 'Correct (Open, Completed, All)' : 'Check configuration'}`);
      console.log(`${threadsExists ? '✅' : '❌'} Threads section: ${threadsExists ? 'Added successfully' : 'Missing'}`);
      console.log(`${!coparentVisible ? '✅' : '❌'} Task filtering: ${!coparentVisible ? 'Working (completed tasks hidden)' : 'Not working (showing completed tasks)'}`);
