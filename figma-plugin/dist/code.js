...
async function createLiaiZenDesignSystem() {
    ...
    try {
        ...
        Object.entries(LIAIZEN_TOKENS.colors.teal).forEach(([name, hex]) => {
            createColorStyle(`Teal/${capitalize(name)}`, hex);
            createdCount.colors++;
        });
        // UI colors
        ...
        for (const [sizeName, sizeValue] of sizes) {
            for (const weight of weights) {
                ...
                await createTextStyle(styleName, LIAIZEN_TOKENS.typography.fontFamily, sizeValue, LIAIZEN_TOKENS.typography.fontWeight[weight]);
                createdCount.textStyles++;
            }
        }
        // 3. CREATE EFFECT STYLES (Shadows)
        for (const [name, value] of Object.entries(LIAIZEN_TOKENS.shadows)) {
            createEffectStyle(`Shadow/${name.toUpperCase()}`, value);
...
async function createVisualStyleGuide() {
    ...
    const existingPages = figma.currentPage.findAll(node => node.type === 'FRAME' && node.name === '🎨 LiaiZen Design System');
...
function createColorGroup(title, colors) {
    ...
    Object.entries(colors).forEach(([name, hex]) => {
        const swatch = createColorSwatch(name, hex);
 ...
 */
function createColorSwatch(name, hex) {
 ...
 */
async function createShadowsSection() {
    const section = figma.createFrame();
    ...
    Object.entries(LIAIZEN_TOKENS.shadows).forEach(([name, value]) => {
        const shadowBox = createShadowExample(name.toUpperCase(), value);
 ...
 */
function createShadowExample(name, shadowValue) {
...
function createSpacingSection() {
    ...
    Object.entries(LIAIZEN_TOKENS.spacing).forEach(([name, value]) => {
        const spacingItem = createSpacingItem(name.toUpperCase(), value);
 ...
 */
function createSpacingItem(name, pixels) {
 ...
 */
async function createTextStyle(name, fontFamily, fontSize, fontWeight) {
    // Check if style already exists
 ...
 */
function createEffectStyle(name, shadowValue) {
    // Check if style already exists
    ...
    colorStyles.forEach(style => {
        ...
        if (paint && paint.type === 'SOLID') {
            ...
            if (name.includes('teal')) {
                if (name.includes('lightest'))
                    tokens.colors.teal.lightest = color;
                else if (name.includes('light'))
                    tokens.colors.teal.light = color;
                else if (name.includes('medium'))
                    tokens.colors.teal.medium = color;
                else if (name.includes('darkest'))
                    tokens.colors.teal.darkest = color;
                else if (name.includes('dark'))
                    tokens.colors.teal.dark = color;
            }
            // Extract semantic colors
            if (name.includes('success'))
                tokens.colors.semantic.success = color;
            else if (name.includes('warning'))
                tokens.colors.semantic.warning = color;
            else if (name.includes('error') || name.includes('danger'))
                tokens.colors.semantic.error = color;
            else if (name.includes('info'))
                tokens.colors.semantic.info = color;
        }
    });
    ...
    if (textStyles.length > 0) {
        ...
        textStyles.forEach(style => {
            ...
            if (size === 12)
                ...
                fontSizeMap.base = '16px';
            else if (size === 18)
                fontSizeMap.lg = '18px';
            else if (size === 20)
                fontSizeMap.xl = '20px';
            else if (size === 24)
                fontSizeMap['2xl'] = '24px';
        });
...
async function exportSelectedComponent() {
    ...
    if ('fills' in node && node.fills && Array.isArray(node.fills)) {
        ...
        if (fills.length > 0) {
            componentData.styles.backgroundColor = rgbToHex(fills[0].color);
        }
    }
    ...
    if (node.type === 'TEXT') {
        ...
        componentData.styles.color = textNode.fills && Array.isArray(textNode.fills) && textNode.fills[0].type === 'SOLID'
            ? rgbToHex(textNode.fills[0].color)
            : '#000';
    }
 ...
 */
async function syncComponentsFromCode(pageType = 'wireframes') {
    try {
        ...
        if (data.success && data.data) {
            ...
            figma.ui.postMessage({
                ...
                components: data.data.components.map((c) => c.name),
            });
 ...
 */
async function createStructureFromData(structure, pageType = 'wireframes') {
    ...
    let targetPage = figma.currentPage.children.find((node) => node.type === 'FRAME' && (node.name.includes('Wireframes') || node.name.includes('Design Pages')));
    if (!targetPage) {
 ...
 */
async function createFrameFromStructure(structure, position, pageType = 'wireframes') {
    ...
    if (structure.fills && Array.isArray(structure.fills)) {
        frame.fills = structure.fills.map((fill) => {
    ...
    if (structure.strokes && Array.isArray(structure.strokes) && structure.strokes.length > 0) {
        frame.strokes = structure.strokes.map((stroke) => {
    ...
    if (structure.effects && Array.isArray(structure.effects)) {
        frame.effects = structure.effects.map((effect) => {
 ...
 */
async function createElementFromStructure(element, position) {
    let figmaElement;
    if (element.type === 'TEXT') {
        ...
        if (element.fills && element.fills.length > 0) {
            ...
            container.fills = element.fills.map((fill) => {
                if (fill.type === 'GRADIENT_LINEAR' && fill.gradientStops) {
                    return {
                        type: 'GRADIENT_LINEAR',
                        gradientStops: fill.gradientStops.map((stop) => ({
        ...
        if (element.strokes && Array.isArray(element.strokes) && element.strokes.length > 0) {
            container.strokes = element.strokes.map((stroke) => {
                if (stroke.type === 'SOLID' && stroke.color) {
                    return { type: 'SOLID', color: stroke.color };
                }
                return stroke;
        ...
        if (element.effects && Array.isArray(element.effects)) {
            container.effects = element.effects.map((effect) => {
                if (effect.type === 'DROP_SHADOW') {
                    return {
                        type: 'DROP_SHADOW',
                        color: effect.color || { r: 0, g: 0, b: 0, a: 0.1 },
