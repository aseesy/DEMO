    ...
    for (const [sizeName, sizeValue] of sizes) {
      for (const weight of weights) {
        ...
        await createTextStyle(
          ...
          sizeValue,
          LIAIZEN_TOKENS.typography.fontWeight[weight]
        );
...
async function createVisualStyleGuide() {
  ...
  const existingPages = figma.currentPage.findAll(
    node => node.type === 'FRAME' && node.name === '🎨 LiaiZen Design System'
  );
...
async function exportSelectedComponent() {
  ...
  if (node.type === 'TEXT') {
    ...
    // Handle text color
    componentData.styles.color =
      textNode.fills && Array.isArray(textNode.fills) && textNode.fills[0].type === 'SOLID'
...
async function createStructureFromData(structure, pageType = 'wireframes') {
  ...
  let targetPage = figma.currentPage.children.find(
    node =>
      node.type === 'FRAME' &&
      (node.name.includes('Wireframes') || node.name.includes('Design Pages'))
  );
  if (!targetPage) {
