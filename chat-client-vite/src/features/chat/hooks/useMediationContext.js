...
export function useMediationContext(username, isAuthenticated) {
  ...
  // Load contacts
  const { contacts, isLoadingContacts, loadContacts } = useContactsApi(username, isAuthenticated);
  ...
  React.useEffect(() => {
    if (isAuthenticated && username && !isLoadingContacts && contacts.length === 0) {
      loadContacts();
    }
  }, [isAuthenticated, username, isLoadingContacts, contacts.length, loadContacts]);
