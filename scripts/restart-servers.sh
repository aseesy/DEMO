...
# Kills and restarts both backend (port 3000) and frontend (port 5173) servers
...
start_backend() {
    ...
    if lsof -ti:3000 > /dev/null 2>&1; then
        echo -e "${GREEN}✅ Backend server started on port 3000 (PID: $BACKEND_PID)${NC}"
...
fix_import_issues() {
    ...
    cd "$PROJECT_ROOT/chat-client-vite/src"
